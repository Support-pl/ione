...
define(function(require) {
  ...
  var lists_month;
  var Colors = {};
  var select_labels = {'cost':true,'cpu':false,'memory':false,'disk':false,'pub_ip':false,'work_time':false};
  ...
  function _setup(context, opt) {
    ...
    showback_dataTable.on("click", "tbody tr", function(){
      ...
      $("#table_test_title .showback_div_label", context).css('border','2px solid #00000036').removeClass('active');
      $("#table_test_title #label_cost", context).css('border','2px solid #4dbbd3').addClass('active');

      $("#table_test_title", context).on('click', '.showback_div_label', function () {
        if (getSelectLabel(this)){
          ...
          for(var j in  vms){
            ...
            $('tr#tr_total:eq(1)').find('td').eq(num).text(sum.toFixed(2));
            num++;
          }
          $('tr#tr_total:eq(1)').find('td').eq(num).text(total.toFixed(2));
          ////////
          test_dataTable.$('tr').css('border-top','1px solid lightgray');
        }
      });

      $("#test_datatable", context).on('click', 'tbody [role="row"]', function () {
        ...
        if ($(this).hasClass("Shown")){
          ...
          $(this).find('td').eq(0).text(day+' '+getWeekDay(new Date(2019,month-1,day)));
      ...
      if (lists[i].TOTAL > 0){
        for(var j in lists[i].showback){
          ...
          if (list_months[month] == undefined){
            ...
            list_months[month][day][i] =  {'cost':cost,'cpu':cpu,'disk':disk,'memory':memor,'pub_ip':pub_ip,'work_time':work_time};
          }else{
          ...
          if (list_months[month][day]['day_total'] == undefined){
            list_months[month][day]['day_total'] = {'cost':cost,'cpu':cpu,'disk':disk,'memory':memor,'pub_ip':pub_ip,'work_time':work_time};
          }else{
          ...
          if (list_months[month]['total'] == undefined){
            list_months[month]['total'] = {'cost':cost,'cpu':cpu,'disk':disk,'memory':memor,'pub_ip':pub_ip,'work_time':work_time};
          }else{
          ...
          if (list_months[month]['vms'] == undefined){
            ...
            list_months[month]['vms'][i] = {'name': lists[i].name,'cost':cost,'cpu':cpu,'disk':disk,'memory':memor,'pub_ip':pub_ip,'work_time':work_time};
          }else{
  ...
  function update_table_template(month){
    ...
    for(var i in lists_month[month]['vms']) {
      $('#test_datatable thead tr').append('<th>'+lists_month[month]['vms'][i].name+'</th>');
    ...
    $('#test_datatable #tr_total').append('<td>'+lists_month[month]['total'].cost.toFixed(2)+'</td>');
    return vms;
  };
  ...
  function more_info(month,day,vms) {
    var more_tr = '<tr hidden><td>CPU<br>Disk<br>Memory<br>Public ip<br>Time</td>';
    for(var i in  vms){
      if (lists_month[month][day][i] != undefined){
        more_tr += '<td>'+ check_data(lists_month[month][day][i].cpu) +
            '<br>'+ check_data(lists_month[month][day][i].disk) +
            '<br>'+ check_data(lists_month[month][day][i].memory) +
            '<br>'+ check_data(lists_month[month][day][i].pub_ip) +
            '<br>'+ check_data(lists_month[month][day][i].work_time) +'</td>';
      }else{
        more_tr += '<td></td>'
      }
    }
    more_tr += '<td>'+ check_data(lists_month[month][day]['day_total'].cpu) +
        '<br>'+ check_data(lists_month[month][day]['day_total'].disk) +
        '<br>'+ check_data(lists_month[month][day]['day_total'].memory) +
        '<br>'+ check_data(lists_month[month][day]['day_total'].pub_ip) +
        '<br>'+ check_data(lists_month[month][day]['day_total'].work_time) +'</td></tr>';
    return more_tr;
  };
  ...
  function getShowback(month,vms) {
    ...
    for(var i in lists_month[month]){
      if (!isNaN(i)){
        ...
        sum = 0;
        for(var l in select_labels){
          if (select_labels[l] == true && lists_month[month][i]['day_total'][l] > 0){
  ...
  function CheckSelectLabels() {
    for(var i in select_labels){
      if (select_labels[i]){
        $('#label_'+i).css('border','2px solid #4dbbd3').addClass('active');
      }else{
        $('#label_'+i).css('border','2px solid #00000036').removeClass('active');
      }
    }
  }
