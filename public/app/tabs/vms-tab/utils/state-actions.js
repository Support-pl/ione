...
define(function(require) {
  ...
  STATE_ACTIONS[OpenNebulaVM.STATES.ACTIVE] = ["VM.recover"];
  ...
  LCM_STATE_ACTIONS[
    OpenNebulaVM.LCM_STATES.DISK_SNAPSHOT_REVERT_SUSPENDED
  ] = [];
  LCM_STATE_ACTIONS[
    OpenNebulaVM.LCM_STATES.DISK_SNAPSHOT_DELETE_SUSPENDED
  ] = [];
  ...
  function enableStateButton(button_action) {
    $(".state-dependent[href='" + button_action + "']")
      .removeAttr("disabled")
      .addClass("vm-action-enabled")
      .removeClass("vm-action-disabled")
      .off("click.stateaction");
          ...
          for (var i in res) {
            if (res[i].ANSIBLE_PROCESS.status == "RUNNING") {
              for (var j in elem) {
                if (
                  elem[j] ==
                  Object.keys(JSON.parse(res[i].ANSIBLE_PROCESS.HOSTS))[0]
                ) {
                  in_proc = true;
                  break;
                }
  ...
  function enabledStateAction(action, state, lcm_state) {
    ...
    return (
      STATE_ACTIONS[state].indexOf(action) != -1 ||
