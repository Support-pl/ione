  ...
  <span>
    <template v-if="typeof value == 'object'">
      <template v-if="opened || status.edit">
        <div
          v-for="[key, val] of entries(value)"
          :key="key"
          :style="getStyle(deepness)"
          class="objectItem"
        >
          <div
            v-if="
              creatingPath == path + '/' + key &&
              status.edit &&
            ...
            <div class="objectBtns objectBtnsBias2btns">
              <div class="btns">
                <a-icon
                  type="plus"
                  class="objectBtn"
                  @click="addKey(path, edit.key, edit.value)"
                />
                <a-icon
                  type="close"
                  class="objectBtn"
                  @click="changeActionPath(null, 'idle')"
                />
              </div>
            </div>
            <a-input
          ...
          <template v-else-if="isEditingKey != key">
            ...
            <div
              v-if="status.edit"
              class="objectBtns objectBtnsBias3btns showOnHoverItemKey"
            >
              <div class="btns">
                <a-icon
                  type="tool"
                  class="objectBtn"
                  @click="update(path, key, val)"
                />
                <a-icon
                  type="plus"
                  class="objectBtn"
                  @click="create(path, key)"
                />
                <a-icon
                  type="minus"
                  class="objectBtn"
                  @click="removeKey(path + '/' + key)"
                />
              </div>
            </div>
            <objectitem
        ...
        <div
          v-if="
            creatingPath == path && status.edit && status.action == 'create'
          "
          :style="getStyle(deepness)"
          class="objectItem"
        >
